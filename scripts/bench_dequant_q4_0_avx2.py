...
def benchmark_child(
    ...
    return {
        "backend": os.environ.get("LIBGGUF_DEQUANT_Q4_0_BACKEND", "auto"),
...
def benchmark_backend(
    ...
    env = os.environ.copy()
    env["LIBGGUF_DEQUANT_Q4_0_BACKEND"] = backend
...
def build_inputs(lib: ctypes.CDLL, rows: int, n_per_row: int, seed: int) -> np.ndarray:
    ...
    row_size = lib.libgguf_row_size(Q4_0_TYPE, n_per_row)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark Q4_0 dequantization kernels.",
    )
