...
template<typename dst_t>
static void dequantize_row_bf16_cuda(const void * vx, dst_t * y, const int k, hipStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
   hipLaunchKernelGGL(( dequantize_block_bf16), dim3(nb), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}

template<typename dst_t>
static void dequantize_row_q1_0_cuda(const void * vx, dst_t * y, const int k, hipStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
   hipLaunchKernelGGL(( dequantize_block_q1_0), dim3(nb), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}

template<typename dst_t>
static void dequantize_row_tq1_0_cuda(const void * vx, dst_t * y, const int k, hipStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
   hipLaunchKernelGGL(( dequantize_block_tq1_0), dim3(nb), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}

template<typename dst_t>
static void dequantize_row_tq2_0_cuda(const void * vx, dst_t * y, const int k, hipStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
   hipLaunchKernelGGL(( dequantize_block_tq2_0), dim3(nb), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}

template<typename dst_t>
static void dequantize_row_mxfp4_cuda(const void * vx, dst_t * y, const int k, hipStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
   hipLaunchKernelGGL(( dequantize_block_mxfp4), dim3(nb), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}

template<typename dst_t>
static void dequantize_row_nvfp4_cuda(const void * vx, dst_t * y, const int k, hipStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
   hipLaunchKernelGGL(( dequantize_block_nvfp4), dim3(nb), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}
