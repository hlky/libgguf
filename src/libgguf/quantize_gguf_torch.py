...
from .quantize_torch_pt import convert_to_gguf, parse_qtype, parse_tensor_qtype
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Torch-native libgguf_torch quantization of safetensors/ckpt diffusion models to GGUF"
    )
    ...
    parser.add_argument("--device", default="cpu", help="Torch device used for quantization, e.g. cpu or cuda")
    parser.add_argument("--compile", action="store_true", help="Wrap libgguf_torch quantization with torch.compile")
    parser.add_argument(
        "--policy",
        choices=("comfy", "dynamic", "uniform"),
        default="comfy",
        help="Tensor selection policy",
    )
    parser.add_argument(
        "--imatrix",
        help=(
            "Unsupported for this torch backend; use quantize-gguf-pt or "
            "quantize-gguf-native for imatrix quantization"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="PATTERN",
        help="Force matching tensors into quantization when possible",
    )
    parser.add_argument(
        "--exclude",
        action="append",
        default=[],
        metavar="PATTERN",
        help="Keep matching tensors unquantized",
    )
    ...
    if args.imatrix:
        parser.error("--imatrix is not supported by the torch quantization backend")
