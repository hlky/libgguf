...
DYNAMIC_ATTENTION_QUERY_PATTERNS = (
    "*.to_q.weight",
    "*.q_proj.weight",
    "*.query_proj.weight",
    "*_attn.q_proj.weight",
...
DYNAMIC_ATTENTION_KEY_PATTERNS = (
    "*.to_k.weight",
    "*.k_proj.weight",
    "*.key_proj.weight",
    "*_attn.k_proj.weight",
...
DYNAMIC_ATTENTION_OUTPUT_PATTERNS = (
    "*.to_out.0.weight",
    "*.to_out.weight",
    "*.o_proj.weight",
    "*.out_proj.weight",
...
DYNAMIC_FFN_GATE_PATTERNS = (
    "*.mlp.gate_proj.weight",
    "*.gate_proj.weight",
    "*.w1.weight",
)
DYNAMIC_FFN_DOWN_PATTERNS = FFN_DOWN_PATTERNS + (
...
def _mixed_policy_qtype(base_file_type: str, base_qtype_name: str, key: str, model_arch: ModelTemplate, counters: dict[str, int]) -> str:
    ...
    if _matches(key, ATTENTION_VALUE_PATTERNS):
        ...
        if base_file_type == "Q3_K_M":
            ...
            qtype = "Q5_1"
        counters["ffn_down"] += 1
    elif model_arch.arch == "aura" and base_file_type == "Q3_K_M" and fnmatchcase(key, "double_layers.*.mlp?.c_proj.weight"):
        qtype = "Q4_K"
    return qtype
    ...
    for key in keys:
        ...
        if forced_qtype is not None:
            ...
            if policy in {"comfy", "dynamic"}:
                target_name = _mixed_policy_qtype(file_type_name, base_qtype_name, key, model_arch, counters)
                if policy == "dynamic":
