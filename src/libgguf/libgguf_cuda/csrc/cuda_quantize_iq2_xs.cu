...
#define GGUF_CUDA_USE_IQ2_XS_NEIGHBOURS
    ...
    for (int ib = 0; ib < QK_K / 16; ++ib) {
        ...
        for (int is = -9; is <= 9; ++is) {
            ...
            for (int k = 0; k < 2; ++k) {
                ...
                int grid_index = gguf_cuda_iq2_find_grid_or_best_neighbour(
                    iq2xs_grid, 512, 2, xval + 8 * k, waux + 8 * k, this_scale, laux + 8 * k, is_on_grid_aux + k);
                (void)grid_index;
            }
        ...
        if ((!is_on_grid[0] || !is_on_grid[1]) && scale > 0.0f) {
            ...
            for (int k = 0; k < 2; ++k) {
                ...
                bool on_grid = false;
                int grid_index = gguf_cuda_iq2_find_grid_or_best_neighbour(
                    iq2xs_grid, 512, 2, xval + 8 * k, waux + 8 * k, scale, l + 8 * k, &on_grid);
                (void)grid_index;
            }
            float sumqx = 0.0f;
            float sumq2 = 0.0f;
