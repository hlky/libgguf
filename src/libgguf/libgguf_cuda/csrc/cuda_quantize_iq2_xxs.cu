...
#define GGUF_CUDA_USE_IQ2_XXS_NEIGHBOURS
    ...
    for (int ib = 0; ib < QK_K / 32; ++ib) {
        ...
        for (int is = -6; is <= 6; ++is) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                int grid_index = gguf_cuda_iq2_find_grid_or_best_neighbour(
                    iq2xxs_grid, 256, 2, xval + 8 * k, waux + 8 * k, this_scale, laux + 8 * k, &on_grid);
                (void)grid_index;
            }
            float sumqx = 0.0f;
            float sumq2 = 0.0f;
        ...
        if (scale > 0.0f) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                bool on_grid = false;
                int grid_index = gguf_cuda_iq2_find_grid_or_best_neighbour(
                    iq2xxs_grid, 256, 2, xval + 8 * k, waux + 8 * k, scale, l + 8 * k, &on_grid);
                (void)grid_index;
            }
            float sumqx = 0.0f;
            float sumq2 = 0.0f;
