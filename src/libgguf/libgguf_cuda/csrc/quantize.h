...
static __global__ void quantize_block_q3_K(const float * __restrict__ x, block_q3_K * __restrict__ y, int64_t n_blocks) {
    ...
    float amax = 0.0f;
    for (int j = 0; j < QK_K / 16; ++j) {
        scales[j] = gguf_cuda_make_q3_quants(16, 4, xb + 16 * j, l + 16 * j);
    ...
    for (int j = 0; j < QK_K / 16; ++j) {
        ...
        for (int ii = 0; ii < 16; ++ii) {
            int q = gguf_cuda_nearest_int(xb[16 * j + ii] / d);
            q = gguf_cuda_clamp_int(q, -4, 3);
            l[16 * j + ii] = q + 4;
        }
    }

    memset(yb->hmask, 0, sizeof(yb->hmask));
    int m = 0;
    uint8_t hm = 1;
...
static __global__ void quantize_block_q4_K(const float * __restrict__ x, block_q4_K * __restrict__ y, int64_t n_blocks) {
    ...
    for (int j = 0; j < QK_K / 32; ++j) {
        float sum_x2 = 0.0f;
        for (int ii = 0; ii < 32; ++ii) {
            const float v = xb[32 * j + ii];
            sum_x2 += v * v;
        }
        const float av_x = sqrtf(sum_x2 / 32.0f);
        for (int ii = 0; ii < 32; ++ii) {
            weights[ii] = av_x + fabsf(xb[32 * j + ii]);
        }
        scales[j] = gguf_cuda_make_qkx2_quants(32, 15, xb + 32 * j, weights, l + 32 * j, &mins[j], laux, -1.0f, 0.1f, 20, false);
        if (scales[j] > max_scale) {
            max_scale = scales[j];
        }
        if (mins[j] > max_min) {
...
static __global__ void quantize_block_q5_K(const float * __restrict__ x, block_q5_K * __restrict__ y, int64_t n_blocks) {
    ...
    for (int j = 0; j < QK_K / 32; ++j) {
        ...
        scales[j] = gguf_cuda_make_qkx2_quants(32, 31, xb + 32 * j, weights, l + 32 * j, &mins[j], laux, -0.5f, 0.1f, 15, false);
        if (scales[j] > max_scale) {
            max_scale = scales[j];
        }
        if (mins[j] > max_min) {
...
static inline void quantize_row_cuda(
    ...
    // K-quant kernels are scalar-thread implementations with high register/stack pressure
    // (see cuobjdump resource usage in bench/results). These launch sizes were selected
    // empirically on SM 8.6 to improve occupancy without changing byte-exact math.
