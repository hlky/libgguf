...
static __device__ __forceinline__ void dequantize_q5_0(const void * vx, const int ib, const int iqs, dfloat2 & v){
    ...
    v.x = (((x[ib].qs[iqs] & 0xf) | xh_0) - 16) * d;
    v.y = (((x[ib].qs[iqs] >>  4) | xh_1) - 16) * d;
}
...
static __global__ void dequantize_block_q3_K(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    const float d_all = __half2float(gguf_cuda_load_half(x[i].d));
    ...
    for (int l = l0; l < l0+4; ++l) {
        y[l] = convert_from_float<dst_t>(dl * ((int8_t)((q[l] >> shift) & 3) - ((hm[l] & m) ? 0 : 4)));
    }
}
...
static __global__ void dequantize_block_iq2_xxs(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    const float d = __half2float(gguf_cuda_load_half(x[i].d)) * (0.5f + (aux32 >> 28)) * 0.25f;
...
static __global__ void dequantize_block_iq3_xxs(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    const float d = __half2float(gguf_cuda_load_half(x[i].d)) * (0.5f + (aux32 >> 28)) * 0.5f;
    const uint8_t signs = ksigns_iq2xs[(aux32 >> 7*il) & 127];
    for (int j = 0; j < 4; ++j) {
...
static __global__ void dequantize_block_iq1_m(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    const float d = __half2float(gguf_cuda_load_half(scale.f16)) * (2*((sc[ib16/4] >> 3*(ib16%4)) & 0x7) + 1);
    const float delta = x[i].qh[2*ib+il/2] & (0x08 << 4*(il%2)) ? -IQ1M_DELTA : IQ1M_DELTA;
    const uint64_t grid = iq1s_grid[x[i].qs[4*ib+il] | (((x[i].qh[2*ib+il/2] >> 4*(il%2)) & 7) << 8)];
    const int8_t * q = (const int8_t *)&grid;
    for (int j = 0; j < 8; ++j) {
        y[j] = d * (q[j] + delta);
    }
...
template<typename dst_t>
static void dequantize_row_bf16_cuda(const void * vx, dst_t * y, const int k, cudaStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_bf16<<<nb, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template<typename dst_t>
static void dequantize_row_q1_0_cuda(const void * vx, dst_t * y, const int k, cudaStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_q1_0<<<nb, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template<typename dst_t>
static void dequantize_row_tq1_0_cuda(const void * vx, dst_t * y, const int k, cudaStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_tq1_0<<<nb, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template<typename dst_t>
static void dequantize_row_tq2_0_cuda(const void * vx, dst_t * y, const int k, cudaStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_tq2_0<<<nb, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template<typename dst_t>
static void dequantize_row_mxfp4_cuda(const void * vx, dst_t * y, const int k, cudaStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_mxfp4<<<nb, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template<typename dst_t>
static void dequantize_row_nvfp4_cuda(const void * vx, dst_t * y, const int k, cudaStream_t stream) {
    const int nb = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_nvfp4<<<nb, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}
...
static to_cuda_ggml_t<dst_t> ggml_get_to_cuda(int64_t type) {
    switch (type) {
        ...
        case 30:
            return dequantize_row_bf16_cuda;
        case 34:
