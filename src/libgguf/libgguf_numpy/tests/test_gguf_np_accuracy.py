...
from ..gguf_np import dequantize, _type_traits
...
ERROR_THRESHOLDS = {
    GGMLQuantizationType.F32: ErrorThresholds(0.0, 0.0, 0.0, 0.999_999),
    GGMLQuantizationType.F16: ErrorThresholds(0.0, 0.0, 0.0, 0.999_999),
...
def test_roundtrip_qtypes_cover_all_numpy_supported_formats() -> None:
    expected = {
        GGMLQuantizationType.F32,
        GGMLQuantizationType.F16,
        *_type_traits.keys(),
    ...
    assert set(ROUNDTRIP_QTYPES) == expected
...
def test_quantize_dequantize_matches_float16_reference(
    ...
    dequantized = dequantize(quantized, qtype)
