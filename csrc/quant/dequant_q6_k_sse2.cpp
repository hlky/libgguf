...
static inline void libgguf_q6_k_store16_sse2(float *RESTRICT y, __m128i q8, __m128 d)
{
...
static inline __m128i libgguf_q6_k_vals_sse2(const uint8_t *RESTRICT ql, const uint8_t *RESTRICT qh, int ql_shift, int qh_shift)
{
...
extern "C" void dequantize_row_q6_K_sse2(const block_q6_K *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    for (int n = 0; n < QK_K; n += 128)
    {
      ...
      {
        const int is = l / 16;
        libgguf_q6_k_store16_sse2(yb + n + l + 0, libgguf_q6_k_vals_sse2(ql + l + 0, qh + l, 0, 0), _mm_set1_ps(d_all * (float)sc[is + 0]));
        libgguf_q6_k_store16_sse2(yb + n + l + 32, libgguf_q6_k_vals_sse2(ql + l + 32, qh + l, 0, 2), _mm_set1_ps(d_all * (float)sc[is + 2]));
        libgguf_q6_k_store16_sse2(yb + n + l + 64, libgguf_q6_k_vals_sse2(ql + l + 0, qh + l, 4, 4), _mm_set1_ps(d_all * (float)sc[is + 4]));
        libgguf_q6_k_store16_sse2(yb + n + l + 96, libgguf_q6_k_vals_sse2(ql + l + 32, qh + l, 4, 6), _mm_set1_ps(d_all * (float)sc[is + 6]));
      }
      ql += 64;
      qh += 32;
      sc += 8;
