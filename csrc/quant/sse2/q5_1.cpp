...
static inline __m128i q5_1_quantize_8_i16_sse2(const float *RESTRICT x, __m128 minv, __m128 id, __m128 half)
{
  const __m128i q0 = _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(_mm_sub_ps(_mm_loadu_ps(x), minv), id), half));
  const __m128i q1 = _mm_cvttps_epi32(_mm_add_ps(_mm_mul_ps(_mm_sub_ps(_mm_loadu_ps(x + 4), minv), id), half));
...
extern "C" void quantize_row_q5_1_sse2(const float *RESTRICT x, block_q5_1 *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    for (int j = 0; j < QK5_1 / 2; j += 8)
    {
      ...
      _mm_store_si128((__m128i *)lo, q5_1_quantize_8_i16_sse2(xb + j, minv, idv, half));
      _mm_store_si128((__m128i *)hi, q5_1_quantize_8_i16_sse2(xb + QK5_1 / 2 + j, minv, idv, half));
