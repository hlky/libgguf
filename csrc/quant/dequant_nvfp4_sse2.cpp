...
static inline void libgguf_nvfp4_store4_sse2(float *RESTRICT y, int v0, int v1, int v2, int v3, __m128 d)
{
  const __m128 q = _mm_set_ps((float)v3, (float)v2, (float)v1, (float)v0);
  _mm_storeu_ps(y, _mm_mul_ps(q, d));
}

extern "C" void dequantize_row_nvfp4_sse2(const block_nvfp4 *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    for (int s = 0; s < QK_NVFP4 / QK_NVFP4_SUB; ++s)
    {
      ...
      {
        libgguf_nvfp4_store4_sse2(yb + j, kvalues_mxfp4[qs[j + 0] & 15], kvalues_mxfp4[qs[j + 1] & 15],
                                  kvalues_mxfp4[qs[j + 2] & 15], kvalues_mxfp4[qs[j + 3] & 15], d);
        libgguf_nvfp4_store4_sse2(yb + j + QK_NVFP4_SUB / 2, kvalues_mxfp4[qs[j + 0] >> 4], kvalues_mxfp4[qs[j + 1] >> 4],
