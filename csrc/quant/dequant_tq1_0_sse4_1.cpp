...
static inline void libgguf_tq1_0_store16_sse41(float *RESTRICT y, __m128i q8, __m128 d)
{
...
static inline void libgguf_tq1_0_store4_sse41(float *RESTRICT y, __m128i q8, __m128 d)
{
  _mm_storeu_ps(y, _mm_mul_ps(_mm_cvtepi32_ps(_mm_cvtepi8_epi32(q8)), d));
}

static inline __m128i libgguf_tq1_0_digits16_sse41(__m128i bytes, int pow3)
{
...
extern "C" void dequantize_row_tq1_0_sse4_1(const block_tq1_0 *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    for (int n = 0; n < 5; ++n)
    {
      libgguf_tq1_0_store16_sse41(yb + out + 0, libgguf_tq1_0_digits16_sse41(p0, pow3[n]), d);
      libgguf_tq1_0_store16_sse41(yb + out + 16, libgguf_tq1_0_digits16_sse41(p1, pow3[n]), d);
      out += 32;
    }
    ...
    for (int n = 0; n < 5; ++n)
    {
      libgguf_tq1_0_store16_sse41(yb + out, libgguf_tq1_0_digits16_sse41(tail, pow3[n]), d);
      out += 16;
    }
    ...
    for (int n = 0; n < 4; ++n)
    {
      libgguf_tq1_0_store4_sse41(yb + out, libgguf_tq1_0_digits16_sse41(qh, qh_pow3[n]), d);
      out += 4;
    }
  }
#else
