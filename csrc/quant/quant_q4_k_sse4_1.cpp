...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_Q4_K_SSE4_1 1
#endif

#if defined(LIBGGUF_BUILD_Q4_K_SSE4_1)
static inline __m128i q4_k_nearest_i32_sse4_1(__m128 v)
{
...
static inline void q4_k_fill_abs_plus_32_sse4_1(const float *RESTRICT x, float av_x, float *RESTRICT weights)
{
...
static inline void q4_k_quantize_32_sse4_1(const float *RESTRICT x, uint8_t *RESTRICT L, float d, float dm)
{
  ...
  for (int ii = 0; ii < 32; ii += 8)
  {
    ...
    __m128i q = _mm_packs_epi32(q4_k_nearest_i32_sse4_1(v0), q4_k_nearest_i32_sse4_1(v1));
...
static inline void q4_k_pack_64_sse4_1(const uint8_t *RESTRICT L, uint8_t *RESTRICT q)
{
  const __m128i low_mask = _mm_set1_epi8(0x0F);
  for (int l = 0; l < 32; l += 16)
  {
    const __m128i lo = _mm_and_si128(_mm_loadu_si128((const __m128i *)(L + l)), low_mask);
    const __m128i hi = _mm_slli_epi16(_mm_and_si128(_mm_loadu_si128((const __m128i *)(L + l + 32)), low_mask), 4);
    _mm_storeu_si128((__m128i *)(q + l), _mm_or_si128(lo, hi));
  }
}
#endif
...
extern "C" void quantize_row_q4_K_sse4_1(const float *RESTRICT x, block_q4_K *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; i++)
  {
    ...
    for (int j = 0; j < QK_K / 32; ++j)
    {
      ...
      q4_k_fill_abs_plus_32_sse4_1(x + 32 * j, av_x, weights);
      scales[j] = libgguf_make_qkx2_quants_sse4_1(32, 15, x + 32 * j, weights, L + 32 * j, &mins[j], Laux, -1.f, 0.1f, 20, false);
    ...
    for (int j = 0; j < QK_K / 32; ++j)
    {
      ...
      q4_k_quantize_32_sse4_1(x + 32 * j, L + 32 * j, d, dm);
    }

    uint8_t *q = y[i].qs;
    for (int j = 0; j < QK_K; j += 64)
    {
      q4_k_pack_64_sse4_1(L + j, q);
      q += 32;
    }
