...
static inline void libgguf_iq3_s_store8_sse4_1(float *RESTRICT y, const uint8_t *RESTRICT grid1, const uint8_t *RESTRICT grid2, uint8_t signs, __m128 db)
{
  ...
  _mm_storeu_ps(y + 0, _mm_xor_ps(_mm_mul_ps(_mm_cvtepi32_ps(_mm_cvtepu8_epi32(g)), db), _mm_castsi128_ps(_mm_loadu_si128((const __m128i *)m0))));
  _mm_storeu_ps(y + 4, _mm_xor_ps(_mm_mul_ps(_mm_cvtepi32_ps(_mm_cvtepu8_epi32(_mm_srli_si128(g, 4))), db), _mm_castsi128_ps(_mm_loadu_si128((const __m128i *)m1))));
}

extern "C" void dequantize_row_iq3_s_sse4_1(const block_iq3_s *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    for (int pair = 0; pair < QK_K / 64; ++pair)
    {
      ...
      for (int l = 0; l < 4; ++l)
      {
        ...
        libgguf_iq3_s_store8_sse4_1(yd + 8 * l, grid1, grid2, signs0[l], db1);
      }
      for (int l = 0; l < 4; ++l)
      {
        ...
        libgguf_iq3_s_store8_sse4_1(yd + 32 + 8 * l, grid1, grid2, signs1[l], db2);
      }
    }
  }
#else
