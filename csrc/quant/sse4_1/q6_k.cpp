...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_Q6_K_SSE4_1 1
#endif

#if defined(LIBGGUF_BUILD_Q6_K_SSE4_1)
static inline __m128i q6_k_nearest_i32_sse4_1(__m128 v)
{
...
static inline void q6_k_quantize_16_sse4_1(const float *RESTRICT x, int8_t *RESTRICT L, float d)
{
  ...
  for (int ii = 0; ii < 16; ii += 8)
  {
    ...
    __m128i q = _mm_packs_epi32(q6_k_nearest_i32_sse4_1(v0), q6_k_nearest_i32_sse4_1(v1));
...
static inline void q6_k_pack_128_sse4_1(const int8_t *RESTRICT L, uint8_t *RESTRICT ql, uint8_t *RESTRICT qh)
{
...
extern "C" void quantize_row_q6_K_sse4_1(const float *RESTRICT x, block_q6_K *RESTRICT y, int64_t k)
{
  ...
  for (int64_t i = 0; i < nb; i++)
  {
    ...
    {
      const float scale = libgguf_make_qx_quants_sse4_1(16, 32, x + 16 * ib, L + 16 * ib, 1, nullptr);
      scales[ib] = scale;
    ...
    for (int j = 0; j < QK_K / 16; ++j)
    {
      ...
      q6_k_quantize_16_sse4_1(x + 16 * j, L + 16 * j, d);
    }
    ...
    {
      q6_k_pack_128_sse4_1(L + j, ql, qh);
      ql += 64;
      qh += 32;
    }
