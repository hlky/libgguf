...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_IQ4_XS_SSE4_1 1
#endif

extern "C" int libgguf_best_index_int8_sse4_1(int n, const int8_t *val, float x);

#if defined(LIBGGUF_BUILD_IQ4_XS_SSE4_1)
static inline void iq4_xs_fill_square_32_sse4_1(const float *RESTRICT x, float *RESTRICT weight)
{
...
static inline void iq4_xs_pack_32_sse4_1(const uint8_t *RESTRICT L, uint8_t *RESTRICT q)
{
...
static bool quantize_one_iq4_xs_sse4_1(const float *RESTRICT x, block_iq4_xs *RESTRICT y)
{
  uint8_t L[QK_K];
  float weight[32];
  ...
  for (int ib = 0; ib < super_block_size / block_size; ++ib)
  {
    ...
    iq4_xs_fill_square_32_sse4_1(xb, weight);
    ...
    for (int j = 0; j < block_size; ++j)
    {
      const int l = libgguf_best_index_int8_sse4_1(16, values, id * xb[j]);
      Lb[j] = (uint8_t)l;
  ...
  for (int ib = 0; ib < super_block_size / block_size; ++ib)
  {
    ...
    for (int j = 0; j < block_size; ++j)
    {
      Lb[j] = (uint8_t)libgguf_best_index_int8_sse4_1(16, values, idl * xb[j]);
    }
  ...
  {
    iq4_xs_pack_32_sse4_1(L + 32 * i, y->qs + 16 * i);
  }
  return true;
}
#endif
...
extern "C" void quantize_row_iq4_xs_sse4_1(const float *RESTRICT x, block_iq4_xs *RESTRICT y, int64_t k)
{
  ...
  {
    if (!quantize_one_iq4_xs_sse4_1(x + i * QK_K, y + i))
    {
      quantize_row_iq4_xs(x, y, k);
      return;
    }
  }
#else
