...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_Q2_K_SSE4_1 1
#endif

#if defined(LIBGGUF_BUILD_Q2_K_SSE4_1)
static inline __m128i q2_k_nearest_i32_sse4_1(__m128 v)
{
...
static inline void q2_k_fill_abs_16_sse4_1(const float *RESTRICT x, float *RESTRICT weights)
{
  const __m128 abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
  for (int l = 0; l < 16; l += 4)
  {
    _mm_storeu_ps(weights + l, _mm_and_ps(_mm_loadu_ps(x + l), abs_mask));
  }
}

static inline void q2_k_quantize_16_sse4_1(const float *RESTRICT x, uint8_t *RESTRICT L, float d, float dm)
{
  ...
  for (int ii = 0; ii < 16; ii += 8)
  {
    ...
    __m128i q = _mm_packs_epi32(q2_k_nearest_i32_sse4_1(v0), q2_k_nearest_i32_sse4_1(v1));
...
static inline void q2_k_pack_128_sse4_1(const uint8_t *RESTRICT L, uint8_t *RESTRICT q)
{
...
extern "C" void quantize_row_q2_K_sse4_1(const float *RESTRICT x, block_q2_K *RESTRICT y, int64_t k)
{
#if defined(LIBGGUF_BUILD_Q2_K_SSE4_1)
  assert(k % QK_K == 0);
  ...
  for (int64_t i = 0; i < nb; i++)
  {
    ...
    {
      q2_k_fill_abs_16_sse4_1(x + 16 * j, weights);
      scales[j] = libgguf_make_qkx2_quants_sse4_1(16, 3, x + 16 * j, weights, L + 16 * j, &mins[j], Laux,
                                                  -0.5f, 0.1f, 15, true);
    ...
    for (int j = 0; j < QK_K / 16; ++j)
    {
      ...
      q2_k_quantize_16_sse4_1(x + 16 * j, L + 16 * j, d, dm);
    }

    for (int j = 0; j < QK_K; j += 128)
    {
      q2_k_pack_128_sse4_1(L + j, y[i].qs + j / 4);
    }

    x += QK_K;
  }
#else
  quantize_row_q2_K(x, y, k);
#endif
}
