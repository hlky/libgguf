...
static inline void iq4_nl_indices_4_sse4_1(const float *RESTRICT x, float id, uint8_t *RESTRICT L)
{
...
static bool iq4_nl_quantize_block_sse4_1(const float *RESTRICT x, block_iq4_nl *RESTRICT y)
{
  uint8_t L[QK4_NL];
  float amax = 0.0f;
  float max = 0.0f;
  ...
  for (int j = 0; j < QK4_NL; j += 4)
    iq4_nl_indices_4_sse4_1(x + j, id, L + j);
...
extern "C" void quantize_row_iq4_nl_sse4_1(const float *RESTRICT x, block_iq4_nl *RESTRICT y, int64_t k)
{
  ...
  {
    if (!iq4_nl_quantize_block_sse4_1(x + i * QK4_NL, y + i))
    {
