...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_Q3_K_SSE4_1 1
#endif

#if defined(LIBGGUF_BUILD_Q3_K_SSE4_1)
static inline __m128i q3_k_nearest_i32_sse4_1(__m128 v)
{
...
static inline void q3_k_quantize_16_sse4_1(const float *RESTRICT x, int8_t *RESTRICT L, float d)
{
  ...
  for (int ii = 0; ii < 16; ii += 8)
  {
    ...
    __m128i q = _mm_packs_epi32(q3_k_nearest_i32_sse4_1(v0), q3_k_nearest_i32_sse4_1(v1));
...
static inline void q3_k_update_hmask_32_sse4_1(int8_t *RESTRICT L, uint8_t *RESTRICT hmask, uint8_t hm)
{
...
static inline void q3_k_pack_128_sse4_1(const int8_t *RESTRICT L, uint8_t *RESTRICT q)
{
...
extern "C" void quantize_row_q3_K_sse4_1(const float *RESTRICT x, block_q3_K *RESTRICT y, int64_t k)
{
#if defined(LIBGGUF_BUILD_Q3_K_SSE4_1)
  assert(k % QK_K == 0);
  ...
  for (int64_t i = 0; i < nb; i++)
  {
    ...
    {
      scales[j] = libgguf_make_q3_quants_sse4_1(16, 4, x + 16 * j, L + 16 * j, true);
    ...
    for (int j = 0; j < QK_K / 16; ++j)
    {
      ...
      q3_k_quantize_16_sse4_1(x + 16 * j, L + 16 * j, d);
    }
    ...
    {
      q3_k_update_hmask_32_sse4_1(L + j, y[i].hmask, hm);
      hm <<= 1;
    }

    for (int j = 0; j < QK_K; j += 128)
    {
      q3_k_pack_128_sse4_1(L + j, y[i].qs + j / 4);
    }
