...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_NVFP4_SSE2 1
#endif

#if defined(LIBGGUF_BUILD_NVFP4_SSE2)
static inline float nvfp4_hmax_sse2(__m128 v)
{
...
static inline bool nvfp4_amax_sub_sse2(const float *RESTRICT x, float *RESTRICT amax)
{
  ...
  const float m = nvfp4_hmax_sse2(maxv);
  if (!isfinite(m))
  {
    return false;
  }
...
static inline __m128i nvfp4_indices4_sse2(const float *RESTRICT x, float d)
{
  const __m128 abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
  ...
  for (int i = 1; i < 16; ++i)
  {
    ...
    best = _mm_or_ps(_mm_and_ps(mask_ps, err), _mm_andnot_ps(mask_ps, best));
    best_index = _mm_or_si128(_mm_and_si128(mask, _mm_set1_epi32(i)), _mm_andnot_si128(mask, best_index));
  }
...
static inline void nvfp4_pack4_sse2(const float *RESTRICT lo_x, const float *RESTRICT hi_x, float d,
                                    uint8_t *RESTRICT q)
{
  const __m128i low_mask = _mm_set1_epi8(0x0F);
  const __m128i lo = _mm_and_si128(nvfp4_indices4_sse2(lo_x, d), low_mask);
  const __m128i hi = _mm_slli_epi16(_mm_and_si128(nvfp4_indices4_sse2(hi_x, d), low_mask), 4);
  const int packed = _mm_cvtsi128_si32(_mm_or_si128(lo, hi));
  memcpy(q, &packed, sizeof(packed));
}

static inline bool nvfp4_quantize_subblock_sse2(const float *RESTRICT x, uint8_t *RESTRICT d_out,
                                                uint8_t *RESTRICT qs)
{
  float amax = 0.0f;
  if (!nvfp4_amax_sub_sse2(x, &amax))
  {
    return false;
  }
  ...
  *d_out = ue;
  ...
  {
    nvfp4_pack4_sse2(x + j, x + QK_NVFP4_SUB / 2 + j, d, qs + j);
  }
  return true;
}
#endif
...
extern "C" void quantize_row_nvfp4_sse2(const float *RESTRICT x, block_nvfp4 *RESTRICT y, int64_t k)
{
  ...
  for (int64_t i = 0; i < nb; ++i)
  {
    for (int s = 0; s < QK_NVFP4 / QK_NVFP4_SUB; ++s)
    {
      ...
      if (!nvfp4_quantize_subblock_sse2(xb, y[i].d + s, qs))
      {
