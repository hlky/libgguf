...
extern "C" void dequantize_row_q5_0_avx2(const block_q5_0 *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    const __m256i lo16 = _mm256_sub_epi16(_mm256_cvtepu8_epi16(lo8), sixteen);
    const __m256i hi16 = _mm256_sub_epi16(_mm256_cvtepu8_epi16(hi8), sixteen);
    ...
    float *yb = y + i * QK5_0;
    _mm256_storeu_ps(yb + 0, _mm256_mul_ps(_mm256_cvtepi32_ps(lo32a), scale));
    _mm256_storeu_ps(yb + 8, _mm256_mul_ps(_mm256_cvtepi32_ps(lo32b), scale));
    _mm256_storeu_ps(yb + 16, _mm256_mul_ps(_mm256_cvtepi32_ps(hi32a), scale));
    _mm256_storeu_ps(yb + 24, _mm256_mul_ps(_mm256_cvtepi32_ps(hi32b), scale));
  }
#else
  GGML_UNUSED(x);
  GGML_UNUSED(y);
