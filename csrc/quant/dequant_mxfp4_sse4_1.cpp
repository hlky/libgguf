...
static inline void libgguf_mxfp4_store4_sse4_1(float *RESTRICT y, int v0, int v1, int v2, int v3, __m128 d)
{
  const __m128 q = _mm_set_ps((float)v3, (float)v2, (float)v1, (float)v0);
  _mm_storeu_ps(y, _mm_mul_ps(q, d));
}

extern "C" void dequantize_row_mxfp4_sse4_1(const block_mxfp4 *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    {
      libgguf_mxfp4_store4_sse4_1(yb + j, kvalues_mxfp4[x[i].qs[j + 0] & 15], kvalues_mxfp4[x[i].qs[j + 1] & 15],
                                  kvalues_mxfp4[x[i].qs[j + 2] & 15], kvalues_mxfp4[x[i].qs[j + 3] & 15], d);
      libgguf_mxfp4_store4_sse4_1(yb + j + QK_MXFP4 / 2, kvalues_mxfp4[x[i].qs[j + 0] >> 4], kvalues_mxfp4[x[i].qs[j + 1] >> 4],
