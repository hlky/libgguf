...
#include <emmintrin.h>
#include <xmmintrin.h>
#define LIBGGUF_BUILD_Q5_K_SSE2 1
#endif

#if defined(LIBGGUF_BUILD_Q5_K_SSE2)
static inline __m128i q5_k_nearest_i32_sse2(__m128 v)
{
...
static inline void q5_k_fill_abs_plus_32_sse2(const float *RESTRICT x, float av_x, float *RESTRICT weights)
{
...
static inline void q5_k_quantize_32_sse2(const float *RESTRICT x, uint8_t *RESTRICT L, float d, float dm)
{
  ...
  for (int ii = 0; ii < 32; ii += 8)
  {
    ...
    __m128i q = _mm_packs_epi32(q5_k_nearest_i32_sse2(v0), q5_k_nearest_i32_sse2(v1));
...
static inline void q5_k_pack_64_sse2(const uint8_t *RESTRICT L, uint8_t *RESTRICT ql, uint8_t *RESTRICT qh,
...
extern "C" void quantize_row_q5_K_sse2(const float *RESTRICT x, block_q5_K *RESTRICT y, int64_t k)
{
  ...
  for (int64_t i = 0; i < nb; i++)
  {
    ...
    for (int j = 0; j < QK_K / 32; ++j)
    {
      ...
      q5_k_fill_abs_plus_32_sse2(x + 32 * j, av_x, weights);
      scales[j] = libgguf_make_qkx2_quants_sse2(32, 31, x + 32 * j, weights, L + 32 * j, &mins[j], Laux,
                                                -0.5f, 0.1f, 15, false);
    ...
    for (int j = 0; j < QK_K / 32; ++j)
    {
      ...
      q5_k_quantize_32_sse2(x + 32 * j, L + 32 * j, d, dm);
    }
    ...
    {
      q5_k_pack_64_sse2(L + n, ql, qh, m1, m2);
      m1 <<= 2;
      m2 <<= 2;
      ql += 32;
    }
