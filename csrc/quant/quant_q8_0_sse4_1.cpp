...
static inline float q8_0_hmax_ps_sse4_1(__m128 v)
{
...
static inline __m128i q8_0_round_away_from_zero_sse4_1(
    __m128 v,
    __m128 id,
    __m128 sign_mask,
    __m128 half,
...
extern "C" void quantize_row_q8_0_sse4_1(const float *RESTRICT x, block_q8_0 *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    const float amax = q8_0_hmax_ps_sse4_1(maxv);
    ...
    {
      const __m128i q0 = q8_0_round_away_from_zero_sse4_1(_mm_loadu_ps(xb + j), idv, sign_mask, half, zero);
      const __m128i q1 = q8_0_round_away_from_zero_sse4_1(_mm_loadu_ps(xb + j + 4), idv, sign_mask, half, zero);
