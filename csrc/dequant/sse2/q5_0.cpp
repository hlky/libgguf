...
extern "C" void dequantize_row_q5_0_sse2(const block_q5_0 *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    const __m128i lo16a = _mm_sub_epi16(_mm_unpacklo_epi8(lo8, zero), sixteen);
    const __m128i lo16b = _mm_sub_epi16(_mm_unpackhi_epi8(lo8, zero), sixteen);
    const __m128i hi16a = _mm_sub_epi16(_mm_unpacklo_epi8(hi8, zero), sixteen);
    const __m128i hi16b = _mm_sub_epi16(_mm_unpackhi_epi8(hi8, zero), sixteen);
