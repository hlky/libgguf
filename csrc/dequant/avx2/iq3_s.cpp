...
static inline void libgguf_iq3_s_store8_avx2(float *RESTRICT y, const uint8_t *RESTRICT grid1, const uint8_t *RESTRICT grid2, uint8_t signs, __m256 db)
{
...
extern "C" void dequantize_row_iq3_s_avx2(const block_iq3_s *RESTRICT x, float *RESTRICT y, int64_t k)
{
  ...
  for (int i = 0; i < nb; ++i)
  {
    ...
    for (int pair = 0; pair < QK_K / 64; ++pair)
    {
      ...
      const __m256 db1 = _mm256_set1_ps(d * (float)(1 + 2 * (x[i].scales[pair] & 0xf)));
      const __m256 db2 = _mm256_set1_ps(d * (float)(1 + 2 * (x[i].scales[pair] >> 4)));
      ...
      for (int l = 0; l < 4; ++l)
      {
        ...
        libgguf_iq3_s_store8_avx2(yd + 8 * l, grid1, grid2, signs0[l], db1);
      }
      for (int l = 0; l < 4; ++l)
      {
        ...
        libgguf_iq3_s_store8_avx2(yd + 32 + 8 * l, grid1, grid2, signs1[l], db2);
      }
    }
  }
#else
