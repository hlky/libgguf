...
#define LIBGGUF_DEQUANT_TYPES(X) \
  ...
  X(Q4_0, q4_0, block_q4_0, sse2)        \
  X(Q8_0, q8_0, block_q8_0, sse2)        \
...
#define WRAP_BACKEND_KERNEL(name, backend, block_type)                                                \
...
#define DEFINE_WRAPPERS(upper, name, block_type, default_backend) \
  ...
  WRAP_BACKEND_KERNEL(name, sse2, block_type)    \
  WRAP_BACKEND_KERNEL(name, sse4_1, block_type)  \
  WRAP_BACKEND_KERNEL(name, avx2, block_type)
