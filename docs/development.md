...
Run each neighbor generator with `--check` before committing changes that affect
IQ lookup construction; check mode compares the tracked header with deterministic
generated content and does not rewrite files. The lookup headers are static,
curated tables rather than generated outputs from these scripts.
