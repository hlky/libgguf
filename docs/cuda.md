...
`--cuda-pipeline 0|1` controls the non-verification CUDA host chunk path. The default `1` uses the two-slot pinned host pipeline; `0` disables that pipeline and uses the serial per-chunk CUDA path without enabling CPU verification. CUDA verification modes always use the serial verified path.

`--verify-cuda-tensors N|all` encodes the first `N` CUDA-routed tensors, or every CUDA-routed tensor, through both CUDA and CPU and compares the encoded bytes. `--verify-cuda-large-tensors N` verifies the `N` largest CUDA-routed tensors by encoded byte size, with deterministic name/index tie-breaking; `0` disables the large-tensor selector. `--verify-cuda-random-tensors N` verifies `N` CUDA-routed tensors selected from stable plan order with `--seed N` and a default seed of `0`; `0` disables the random selector, and counts above the CUDA-routed tensor count verify all available candidates. When multiple verification selectors are supplied, the converter verifies their union and reports each verified CUDA tensor once in `cuda_verified`. `--timings` reports `read`, `cpu_convert`, `h2d`, `cuda_quant`, `d2h`, `write`, and `total` buckets, plus metadata, `cuda_pipeline=0|1`, and tensor counts including `cuda_chunks`.
