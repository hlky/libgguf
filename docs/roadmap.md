...
The native converter already contains GGUF write and safetensors payload logic, and `gguf-inspect`/`gguf-validate` now expose header/metadata/tensor descriptor inspection plus minimal structural validation. The roadmap item is to expose a cleaner public reader/writer surface rather than asking downstream code to depend on converter internals.
